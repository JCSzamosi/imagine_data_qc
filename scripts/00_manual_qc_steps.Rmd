...
```{r}
samp_inf = read.csv(file = 'data/active_Surette_sample_info_sheet.csv'
                    strip.white = TRUE)
dim(samp_inf)
```
...
There are 61 negative controls. I will partition them off for now
...
Now let's look for any IMG ids that are missing from the sample data sheet
...
There are 46 samples that start with IMG but are missing from the sample info
