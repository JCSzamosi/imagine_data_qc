...
mapfile = 'merged_maptab.rds'

clstab = read.csv(file.path(indir, clstabf), row.names = 1)
taxtab = read.csv(file.path(indir, taxtabf), row.names = 1)
maptab = readRDS(file.path(datdir, mapfile))

cat('\nCheck the sequences\n')
rownames(taxtab) = taxtab$seqs
taxtab = taxtab[,-seqs]

if (!all(rownames(taxtab) %in% rownames(clstab))){
	msg = 'Some sequences in the tax table are missing from the counts table'
	stop(msg)
}

if (!all(rownames(clstab) %in% rownames(taxtab))){
	msg = 'Some sequences in the counts table are missing from the tax table'
	stop(msg)
}
