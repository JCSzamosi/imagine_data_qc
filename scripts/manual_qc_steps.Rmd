...
```{r}
write.csv(samp_dup, file = 'intermed/Rossi_sample_info_duplicate_SampleIDs.csv')
```
...
meta_only = meta_long$Sample.ID[!meta_long$Sample.ID %in% samp_inf$Sample.ID]
length(meta_only)
```

Nope. There are 42 Sample IDs in the metadata that are absent from the sample
info sheet:

```{r}
meta_only
meta_only = filter(meta_long, Sample.ID %in% meta_only)
meta_only
```

I will ask Michelle and Laura about these.

```{r}
write.csv(meta_only, file = 'intermed/IMAGINE_samps_missing_from_info_sheet.csv')
```
...
There are 70 samples in the sample info sheet that are missing from the metadata:
...
5 of these are discards, leaving 65:
...
I will ask Michelle and Laura:

```{r}
write.csv(samp_only, file = 'intermed/SuretteLab_samps_missing_from_IMAGINE_sheet.csv')
```
